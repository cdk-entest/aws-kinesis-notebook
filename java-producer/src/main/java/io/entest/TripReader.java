...
public class TripReader {
  ...
  public static List<Trip> readFile(File csvFile) throws Exception {
    ...
    // schema from the Trip class  
    CsvSchema schema = mapper.schemaFor(Trip.class).withSkipFirstDataRow(true);

    // read csv file and return iterator 
    MappingIterator<Trip> iterator =
        mapper.readerFor(Trip.class).with(schema).readValues(csvFile);

    // iterate and return list of object 
    List<Trip> tripsToReturn = iterator.readAll().stream().map(x -> {
      try {
  ...
  public static Trip modifyIncomingDates(Trip trip) throws ParseException {
    ...
    if (firstDate == null) {
      ...
      timeDeltaBetweenFirstDateAndNow =
          Instant.now().getEpochSecond() * 1000 - firstDate.getMillis();
    }

    trip.pickupDate = outputFormat
        .print(pickupDateObject.getMillis() + timeDeltaBetweenFirstDateAndNow);
    trip.dropoffDate = outputFormat
        .print(dropoffDateObject.getMillis() + timeDeltaBetweenFirstDateAndNow);
